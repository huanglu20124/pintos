...
bool
compare(const struct list_elem*elem1, const struct list_elem*e,void*aux UNUSED)
{
      return  list_entry(elem1, struct thread, elem)->priority >
              list_entry(e, struct thread, elem)->priority;
}

/*void list_insert_sorted_lock_priority(struct list*list, struct list_elem*elem1)
{
     ASSERT (list != NULL);
     ASSERT (elem1 != NULL);
     struct list_elem*e;
     ...
     {      
             struct lock* l1 = list_entry(elem1, struct lock, holder_elem);
             struct lock* l2 = list_entry(e, struct lock, holder_elem);
             if(l1->lock_priority > l2->lock_priority)
             break;
     }
     return  list_insert(e,elem1);	
} */

void list_insert_sorted(struct list*list, struct list_elem*elem1)
{
     ASSERT (list != NULL);
     ASSERT (elem1 != NULL);
     struct list_elem*e;
